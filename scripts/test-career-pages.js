...
assert.strictEqual(indexPage.data.exploration.total, 360);
...
assert.strictEqual(playPage.data.view.runExploration.shownCount, 36);
assert(playPage.data.view.runExploration.newRate >= 0);
...
assert.strictEqual(archivePage.data.discoveries.total, 360);
...
assert(indexWxml.includes("自由模拟复玩时会优先安排未见经历"));
assert(indexWxml.includes("题序固定，不根据个人探索记录调整"));
...
assert(playWxml.includes("首次遇见"));
assert(playWxml.includes("本轮遇见 {{view.runExploration.newCount}} 个新事件"));
