...
const departures = require("../miniprogram/utils/departureStore");
...
assert.strictEqual(first.bookingIds.length, 1);
assert.strictEqual(first.checklistItemIds.length, departures.CHECKLIST_TEMPLATES.length);
...
assert.strictEqual(demo.getTargetId("departure"), first.bookingIds[0]);
...
assert.strictEqual(departures.getBookingById(second.bookingIds[0]), null);
assert.strictEqual(departures.getChecklistItems().some((item) => second.checklistItemIds.indexOf(item.id) >= 0), false);
