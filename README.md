...
- 支持“自由生涯”和“今日挑战”：自由生涯每次抽取新事件，同一天的今日挑战使用固定种子。
...
- 生涯档案保留最终属性、职业画像、关键词、关键选择和结局，支持收集 `12` 种职业结局与 `12` 项跨周目成就。
- 结局页和历史档案可复制纯文字生涯总结，便于与朋友比较同题选择。
...
数据设置页可以导出和导入完整 JSON 备份。当前工具箱备份格式为 `schemaVersion: 2`，与 AA 账本自身的 `schemaVersion: 4`、游戏生涯自身的 `schemaVersion: 2` 是不同层级的版本号。
...
miniprogram/
  ...
  packages/tools/utils/
    ...
    careerGameMeta.js            今日挑战、职业画像、章节报告和成就
...
- 游戏 60 个事件的结构引用、自由/每日种子复现、属性边界、隐藏经历、延迟后果和 12 种结局。
- 游戏动态职业画像、章节净变化、伏笔提示、12 项成就和旧存档升级。
- 游戏自动存档、结果阶段幂等、章节推进、重开确认、复制总结、生涯档案和双图鉴页面事件。
