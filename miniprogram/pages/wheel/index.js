...
const COLORS = ["#2f6f6d", "#e36f50", "#e3b341", "#4a75bd", "#bd4f5c", "#67a05b", "#8b6fb0", "#3d8ea3"];
...
Page({
  ...
  draw() {
    ...
    ctx.clearRect(0, 0, width, height); ctx.save(); ctx.translate(cx, cy); ctx.rotate(this.rotation);
    if (!options.length) { ctx.beginPath(); ctx.arc(0, 0, radius, 0, engine.TAU); ctx.fillStyle = "#e8ebef"; ctx.fill(); ctx.fillStyle = "#667085"; ctx.font = "600 14px sans-serif"; ctx.textAlign = "center"; ctx.fillText("先添加至少两个选项", 0, 5); ctx.restore(); return; }
    ...
    options.forEach((option, index) => {
      ...
      ctx.beginPath(); ctx.moveTo(0, 0); ctx.arc(0, 0, radius, start, end); ctx.closePath(); ctx.fillStyle = COLORS[index % COLORS.length]; ctx.fill(); ctx.strokeStyle = "rgba(255,255,255,.72)"; ctx.lineWidth = 1; ctx.stroke();
    ...
    ctx.beginPath(); ctx.arc(0, 0, Math.max(18, radius * 0.11), 0, engine.TAU); ctx.fillStyle = "#172033"; ctx.fill(); ctx.restore();
  },
  ...
  addBatch() { if (this.data.spinning) return; if (!this.data.batchText.trim()) return wx.showToast({ title: "请输入选项", icon: "none" }); store.addOptions(this.data.wheel.id, this.data.batchText); this.setData({ batchText: "" }); this.loadWheels(this.data.wheel.id); },
  ...
  closeWinner() { this.setData({ winner: null, showEditor: true }); },
  spinAgain() { this.setData({ winner: null }, () => this.spin()); },
  removeWinner() { if (!this.data.winner) return; try { store.removeOption(this.data.wheel.id, this.data.winner.optionId); this.setData({ winner: null, showEditor: true }); this.loadWheels(this.data.wheel.id); } catch (error) { wx.showToast({ title: error.message, icon: "none" }); } },
