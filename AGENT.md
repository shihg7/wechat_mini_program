...
本文是开发者和编程 Agent 进入仓库后的第一份操作说明。它记录当前产品边界、代码结构、数据约束、验证流程，以及微信开发者工具官方 Skills 的学习和使用方式。
...
- 华子研发模拟使用 `huaweiSimProgressStore.js`。
- 只保存已见题编号、出现次数、最近题目和完成轮数。
- 不保存具体选择、反馈或结果。
...
- 当前有 48 个词条、64 个情景、192 个选择。
- 首轮完成后解锁 10 个复玩支线。
- 后续依次优先未见题、低频题、最近未出现的题。
- 题池容量允许时，前四轮完整模拟不重复。
- 当前选择和结果不落盘，只有轻量探索进度保存在本机。
