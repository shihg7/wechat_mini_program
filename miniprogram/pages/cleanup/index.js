...
Page({
  ...
  ignoreDuplicate(event) { cleanupService.ignoreDuplicate(event.currentTarget.dataset.source, event.currentTarget.dataset.target); this.refresh(); },
  clearInvalidPhotos(event) {
    wx.showModal({ title: "清理失效照片", content: "只删除已经无法读取的照片元数据，不影响仍存在的照片。", confirmText: "清理", success: (result) => { if (!result.confirm) return; cleanupService.removeInvalidPhotoMetadata(event.currentTarget.dataset.id); this.refresh(); } });
  },
  clearOrphans() {
