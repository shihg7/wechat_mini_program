...
一个完全离线的微信小程序日常工具箱。首页平级提供八个工具：日期计算、单位换算、二维码生成、决策转盘、AA 分账、简版行程、通用清单和程序员生涯模拟；右上角仅保留帮助与数据设置。

应用不要求登录，也不依赖业务服务器。日期、换算和二维码输入只存在当前页面，退出即清空；其余五类工具数据保存在微信小程序本地缓存中，彼此不保存关联 ID。
...
- 事件库扩展为 `120` 个职业情景、`355` 个选择；每章保留 `4` 个固定事件，并从 `16` 个支线中按种子与经历抽取 `2` 个，单次模拟仍经历 `36` 个抉择。
- 新支线覆盖 AI 协作、供应链安全、无障碍、隐私伦理、跨国远程、裁员、创业、家庭照护、开源治理和职业传承等主题，事件卡会显示主题标签。
- 支持“自由模拟”和“今日情景”：自由模拟每次抽取新事件，同一天的今日情景使用固定种子。
...
- 生涯档案保留最终属性、职业画像、关键词、关键选择和职业答案，可发现 `12` 种职业答案、`12` 项跨生涯里程碑，并显示 `120` 个事件的图鉴进度。
...
五类持久工具通过本地域 Store 读写数据，不直接操作业务缓存；三个临时工具没有 Store，也不进入备份：
...
2. 确认模拟器能进入八工具首页，控制台没有 WXML、WXSS、JS 或路由错误。
3. 依次点击八个工具；日期、换算和二维码至少完成一次有效输入与结果操作。
...
- 生涯模拟 120 个事件、355 个选择的结构引用、主题覆盖、随机池可达性、自由/每日种子复现、条件回响、属性边界、延迟后果和 12 种职业答案。
