...
本文是开发者和编程 Agent 进入仓库后的第一份操作说明。它记录当前 `1.11.4` 产品边界、代码结构、数据约束、验证流程，以及微信开发者工具官方 Skills 的学习和使用方式。
...
- 名称候选必须来自头像附近的稀疏文字形态，使用稳健水平边界剔除少量孤立纹理点并收紧到实际内容；私聊气泡、聊天图片和纹理壁纸必须有反误判及漏检测试。
