...
Page({
  data: { trips: [], groups: [], keyword: "", activeStatus: "all", statuses: [{ key: "all", label: "全部" }].concat(GROUPS) },
  onShow() { this.load(); },
  load() {
    const keyword = this.data.keyword.trim().toLowerCase();
    const trips = tripStore.getTrips().filter((trip) => (this.data.activeStatus === "all" || trip.status === this.data.activeStatus) && (!keyword || [trip.title, trip.cities.join("、"), trip.note].join(" ").toLowerCase().indexOf(keyword) >= 0));
