...
本文是开发者和编程 Agent 进入仓库后的第一份操作说明。它记录当前 `1.11.5` 产品边界、代码结构、数据约束、验证流程，以及微信开发者工具官方 Skills 的学习和使用方式。
...
- 禁止只用像素复杂度或颜色方差猜头像；先识别聊天内容上下边界并排除底部输入工具栏，头像候选再同时检查边缘位置、至少三侧稳定方形边界、紧邻内侧的消息内容证据和消息起始位置。
- 远处气泡、高图片或表格中段、时间条和另一侧消息不得为边缘壁纸或工具栏图标提供头像证据；必须保留左右两侧的反误判测试。
- 名称候选必须来自头像内侧紧邻的稀疏文字形态，使用稳健水平边界剔除居中时间和孤立纹理点并收紧到实际内容；低对比度图片消息昵称、私聊气泡、聊天图片和纹理壁纸必须有反误判及漏检测试。
- 自动结果只是候选，界面必须明确要求用户检查；手工框选和涂抹始终可用。
- 区域统一使用原图归一化坐标，预览缩放、移动和原图导出不得各自维护另一套坐标。
- 马赛克和模糊强度不得低于隐私安全下限；多个效果重叠时，更彻底的纯色遮挡必须最后绘制。
- 效果控件采用明确的范围语义：没有选区时批量修改全部区域，选中边框后只修改所选区域；滑块拖动必须实时预览，松手只产生一条撤销记录。
...
- 首页十个入口都能进入并返回。
- 日期、换算、二维码各完成一次有效输入。
- 截图打码至少验证浅色双侧、深色单侧、私聊无昵称、群聊低对比度昵称、居中时间、高图片或表格中段、纹理壁纸、底部输入工具栏、远处气泡、不同头像尺寸和长截图；检查自动候选，手工补充一处，并确认导出 PNG 与方向校正后的原图尺寸一致。
