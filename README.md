...
一个完全离线的微信小程序日常工具箱。首页平级提供五个工具：AA 分账、简版行程、通用清单、决策转盘、酒店餐厅快评；右上角仅保留帮助与数据设置。
...
数据设置页可以导出和导入完整 JSON 备份。新工具箱备份格式从 `schemaVersion: 1` 重新开始，与 AA 账本自身的 `schemaVersion: 4` 是两个不同层级的版本号。
...
第一次运行工具箱版本时，`toolboxMigration` 会执行一次不可逆清理：删除旧版体验档案缓存及其中登记的本地照片，然后写入 `toolbox_initialized_v1` 标记。后续启动不会重复执行，也不会删除上述五个 `toolbox_*` 新缓存。
...
2. 确认模拟器能进入五工具首页，控制台没有 WXML、WXSS、JS 或路由错误。
3. 依次点击五个工具，完成一次新增、编辑、返回和删除流程。
...
- 完整备份预检、合并与覆盖、重复 ID、非法文件和写入失败回滚。
- 首次升级清理只执行一次，并保护五个新工具缓存。
- 页面路由、主包边界、图标资源、工具分包和用户手册一致性。

每次提交前至少执行 `npm run verify`；涉及界面或路由时，还应完成微信开发者工具的普通编译、代码质量扫描和主要点击流程。
