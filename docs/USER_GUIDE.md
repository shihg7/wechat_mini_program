...
2. **分清临时与保存：** 日期、换算、二维码和华子研发模拟退出即清空；AA、行程、清单、转盘和程序员生涯模拟会保存到本地。
...
这是非官方的研发文化情景模拟。40 个词条整理自企业公开材料、公开管理资料和行业通用表达；44 个遭遇全部是虚构复合创作，不代表任何企业、团队或员工的真实经历。每次从四个阶段抽取 12 个情景，只在当前页面运行。
...
1. **开始一次模拟：** 系统按种子从入场对齐、版本攻坚、一线协同和路线抉择中各抽取三个情景。
2. **阅读人话翻译：** 每个情景会关联一个词条，并把缩写或文化表达翻译成通俗含义。
3. **做出不能重复结算的选择：** 选项会改变交付信用、技术深度、能量余量和组织影响，反馈出现后才能进入下一题。
4. **查看研发画像：** 十二次选择结束后生成本地画像、路线关键词、四项数值和完整现场记录。
5. **按需查询词典：** 词典支持关键词与分类筛选，可以复制解释，但不会保存搜索内容。
...
数据设置集中显示本地占用和上次备份时间，并提供完整导出、恢复和清空。备份文件包含五类持久工具数据，不包含日期、换算、二维码和华子研发模拟四项临时工具的输入。
