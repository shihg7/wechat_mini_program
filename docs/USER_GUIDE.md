...
> 适用版本：微信小程序纯前端版 1.11.0（备份格式 v3）<br>
> 更新日期：2026-08-01<br>
...
选择一张聊天截图后，工具会依据左右聊天布局标记头像、邻近名称和顶部标题。识别不读取姓名文字，也不会上传图片，保存前仍需人工检查。
...
2. **检查自动标记：** 绿色边框表示自动候选。逐一检查双方头像、群聊名称和顶部标题是否存在遗漏或误判。
3. **补充或调整：** 使用移动模式选择、移动、缩放或删除区域；使用框选或涂抹补充需要隐藏的内容。
4. **选择遮挡效果：** 可使用马赛克、模糊或黑灰白纯色；所选区域也可以临时停用或重新启用。
...
自动识别只依据常见聊天布局生成候选，不读取文字或识别人脸。请切换到框选或涂抹模式补充遗漏后再保存。
