...
本文是开发者和编程 Agent 进入仓库后的第一份操作说明。它记录当前 `1.11.1` 产品边界、代码结构、数据约束、验证流程，以及微信开发者工具官方 Skills 的学习和使用方式。
...
- 禁止只用像素复杂度或颜色方差猜头像；头像候选必须同时检查边缘位置、方形边界和向内侧延伸的消息内容证据。
...
- 首页十个入口都能进入并返回。
- 日期、换算、二维码各完成一次有效输入。
- 截图打码至少验证浅色双侧、深色单侧、私聊无昵称、群聊昵称、聊天大图、纹理壁纸、不同头像尺寸和长截图；检查自动候选，手工补充一处，并确认导出 PNG 与方向校正后的原图尺寸一致。
