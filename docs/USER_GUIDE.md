...
4. [二维码生成](#section-qr)
5. [AA 分账](#section-ledger)
6. [行程安排](#section-trips)
7. [通用清单](#section-checklists)
8. [决策转盘](#section-wheel)
9. [程序员生涯模拟](#section-career)
10. [华子研发模拟](#section-huawei)
11. [本地数据与备份](#section-data)
12. [常见问题](#section-faq)
...
首页提供九个平级工具，右上角保留帮助和数据设置。所有功能都可离线使用，不需要注册账号。
...
2. **分清临时与保存：** 日期、换算和二维码退出即清空；华子研发模拟会清空本轮选择和结果，只保留已见题进度；AA、行程、清单、转盘和程序员生涯模拟会保存到本地。
...
从入行求职走到职业答案，在六个阶段中做出不能撤销的选择。120 个职业情景覆盖 AI、安全、伦理、开源、管理和人生变量；五项属性、隐藏经历和延迟后果会共同影响事件走向与最终答案。结果来自预设情景和本地规则，仅用于选择反思，不构成职业建议。
...
1. **选择模拟模式：** 自由模拟每次抽取新事件；今日情景在同一天使用固定种子，方便对照不同选择。
...
两种模式都从入行求职开始，依次经历初级生存、独当一面、核心骨干、路线分叉和职业答案。事件库共有 120 个情景，每章固定经历 4 个主线，并从 16 个支线中抽取 2 个；因此一局仍是 36 个抉择。相同种子配合相同选择会得到一致结果，但选择触发的隐藏经历仍会改变后续候选事件。
...
这是非官方的研发文化情景模拟。48 个词条整理自企业公开材料、公开管理资料、行业通用表达和单独标记的网络职场叙事；64 个遭遇全部是虚构复合创作，不代表任何企业、团队或员工的真实经历。每次从五个阶段抽取 15 个情景，首轮完成后解锁 10 个复玩支线，后续优先安排未见和较少出现的内容。
...
5. **继续探索新内容：** 完成首轮后会解锁十个复玩专属情景；再次进入时，系统会优先选择未见题，再选择较少出现且不在最近记录中的题。
...
数据设置集中显示本地占用和上次备份时间，并提供完整导出、恢复和清空。备份文件包含五类持久工具数据，不包含日期、换算、二维码的临时输入，也不包含华子研发模拟的选择、结果或轻量探索进度。
...
4. **谨慎清空：** 清空会一次删除五类持久工具的本地内容，操作前务必确认已有可用备份。
...
遇到问题时，先确认当前工具、输入内容和最近一次备份状态。九个工具彼此独立，可以分别排查。
